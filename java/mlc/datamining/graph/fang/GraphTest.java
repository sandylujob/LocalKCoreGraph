...
public class GraphTest {
	public static void main(String[] args) throws Exception 
	{
		...
		BaseGraph graph = new HyperGraph("C:/Users/sandy/Documents/MATLAB/javagraph/data/hypergraph/", "FangYao.txt");
...
//		int countV = 20;
//		int countE = 30;
//		BaseGraph graph = new GeneratedGraph(countV, countE, 10000000);
//		graph.focusOnGC();
//		graph.viewNetwork();
...
//		BaseGraph graph=new GeneratedGraph(GeneratedGraph.TYPE_POWERLAW_BARABASIALBERT, 10, 5, 3);
//		graph.removeByEdgeDegrees(50);
		graph.focusOnGC();
		graph.viewNetwork();
		...
		//random graph
//		for(int numOfV = 50; numOfV<40000; numOfV += numOfV) {
//			double maxConnectionProbility = 0.5;
//			for(double probility=0.001; probility<maxConnectionProbility; probility+=probility) {
//				double totalEdge = 0;
//				double totalDegree = 0;
//				double totalComponentDegree = 0;
//				double totalNormalizedComponentDegree = 0.0;
//				for(int i=0; i<interation; i++) {
////					graph=new GeneratedGraph(numOfV, numOfE, 1000000);
//					graph=new GeneratedGraph(GeneratedGraph.TYPE_RANDOM_ERDOSRENYI, numOfV, probility);
...
//					totalEdge += graph.edgeList.size();
//					
////					System.out.println("(" + n + ", " + m + ")\t" + 
////							i + "\t" + vertex + "\t" + graph.network.getNeighborCount(vertex) + "\t" + componentDegreeMap.get(vertex));
//				}
//				totalEdge /= interation;
//				System.out.println(numOfV + "\t" + BaseGraph.formatInteger.format(totalEdge) + "\t" 
//						+ BaseGraph.formatPercentage.format(totalEdge/(numOfV*numOfV)) + "\t"
...
//		}
		//power law graph
		for(int numOfV = 50; numOfV<10000; numOfV += numOfV) {
			int maxNumOfStep = numOfV * numOfV / 2 ;
			for(int numOfStep=1; numOfStep<maxNumOfStep; numOfStep+=numOfStep) {
				...
				double totalNormalizedComponentDegree = 0.0;
				for(int i=0; i<interation; i++) {
					graph=new GeneratedGraph(GeneratedGraph.TYPE_POWERLAW_BARABASIALBERT, numOfV, (int)Math.sqrt(numOfV), numOfStep);
					ArrayList<Number> vertexList = new ArrayList<Number>(graph.vertexList);
					Collections.sort(vertexList, graph.degreeComparator);
					Collections.reverse(vertexList);
					Number vertex = vertexList.get(0);
					...
					totalComponentDegree += componentDegree;
					totalNormalizedComponentDegree += componentDegree / degree;
					totalVertex += graph.vertexList.size();
					totalEdge += graph.edgeList.size();
//					System.out.println("(" + n + ", " + m + ")\t" + 
//							i + "\t" + vertex + "\t" + graph.network.getNeighborCount(vertex) + "\t" + componentDegreeMap.get(vertex));
				}
				totalVertex /= interation;
				totalEdge /= interation;
				System.out.println(BaseGraph.formatInteger.format(totalVertex) + "\t" 
						+ BaseGraph.formatInteger.format(totalEdge) + "\t" 
						+ BaseGraph.formatPercentage.format(totalEdge/(totalVertex*totalVertex)) + "\t"
						+ BaseGraph.formatDecimal.format(totalDegree/interation) + "\t"
						+ BaseGraph.formatDecimal.format(totalComponentDegree/interation) + "\t" 
						+ BaseGraph.formatScientific.format(totalNormalizedComponentDegree/interation));
...
//		for(int numOfV = 50; numOfV<40000; numOfV += numOfV) {
//		int maxNumOfE = numOfV * numOfV / 2;
//		for(int numOfE=numOfV; numOfE<maxNumOfE; numOfE+=numOfE) {
//			double totalDegree = 0;
//			double totalComponentDegree = 0;
//			double totalNormalizedComponentDegree = 0.0;
//			for(int i=0; i<interation; i++) {
//				graph=new GeneratedGraph(GeneratedGraph.TYPE_POWERLAW_EPPSTEIN, numOfV, numOfE);
//				ArrayList<Number> vertexList = new ArrayList<Number>(graph.vertexList);
//				Collections.sort(vertexList, graph.degreeComparator);
//				Collections.reverse(vertexList);
//				Number vertex = vertexList.get(0);
